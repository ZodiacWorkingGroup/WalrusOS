...
def createchar(canvas, commands, x, y, color='black', scalar=1, font_weight=0):
    ...
    for com in commands:
        ...
        args = com[1:]
        if ins == 'line':
            r.append(canvas.create_line((x+args[0])*scalar, (y-args[1])*scalar, (x+args[2])*scalar, (y-args[3])*scalar,
                                        fill=color, width=size, cap=PROJECTING))

        elif ins == 'rect':
            r.append(canvas.create_rectangle((x+args[0])*scalar, (y-args[1])*scalar, (x+args[2])*scalar,
                                             (y-args[3])*scalar, outline=color))

        elif ins == 'fillrect':
            r.append(canvas.create_rectangle((x+args[0])*scalar, (y-args[1])*scalar, (x+args[2])*scalar,
                                             (y-args[3])*scalar, outline=color, fill=color))

        elif ins == 'circle':
            r.append(rend.draw_circle(canvas, x+args[0], y-args[1], args[2],
                                      fill=color, scalar=scalar, width=size, cap=PROJECTING))

        elif ins == 'arc':
            r.append(rend.draw_arc(canvas, x+args[0], y-args[1], args[2], args[2], args[3], args[4], 0,
                                   fill=color, scalar=scalar, width=size, cap=PROJECTING))

        elif ins == 'ellipse':
            r.append(rend.draw_ellipse(canvas, x+args[0], y-args[1], args[2], args[3], args[4],
                                       fill=color, scalar=scalar, width=size, cap=PROJECTING))

        elif ins == 'ellarc':
            r.append(rend.draw_arc(canvas, x+args[0], y-args[1], args[2], args[3], args[4], args[5], args[6],
                                   fill=color, scalar=scalar, width=size, cap=PROJECTING))
